...
def generateExternalFunction(pathOpenSimModel, outputDir, pathID,
    ...
    with open(pathOutputFile, "w") as f:
        ...
        nOutputs = nCoordinates + 3*(2+2+nBodies) + 1 #CL added +1 for COM_vel_x
        ...
        # CL ADDED THIS NEW SECTION:
        # Compute whole-body COM position and velocity
        f.write('\t/// Whole-body center of mass position and velocity.\n')
        f.write('\tdouble total_mass = 0.0;\n')
        f.write('\tVec3 COM_pos(0.0, 0.0, 0.0);\n')
        f.write('\tVec3 COM_vel(0.0, 0.0, 0.0);\n')
        f.write('\n')

        # Calculate total mass and COM position
        for i in range(bodySet.getSize()):        
            c_body = bodySet.get(i)
            c_body_name = c_body.getName()            
            if (c_body_name == 'patella_l' or c_body_name == 'patella_r'):
            ...
            f.write('\t/// Contribution from %s\n' % c_body_name)
            f.write('\ttotal_mass += %.20f;\n' % c_body_mass)
            f.write('\tCOM_pos += %.20f * %s->getPositionInGround(*state);\n' % (c_body_mass, c_body_name))
            f.write('\tCOM_vel += %.20f * %s->getVelocityInGround(*state)[1];\n' % (c_body_mass, c_body_name))
            f.write('\n')

        # Normalize by total mass
        f.write('\tCOM_pos /= total_mass;\n')
        f.write('\tCOM_vel /= total_mass;\n')
        f.write('\n')
